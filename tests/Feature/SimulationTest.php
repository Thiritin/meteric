...
it('simulates a full monthly cycle S → M → S with prepaid discard downgrade', function () {
    ...
    $sub = Billify::subscribe()->account($acc)->at(CarbonImmutable::parse('2026-06-01T00:00:00Z'))->add($s, 1)->create();
    ...
    Billify::changePlan($item, $m, at: CarbonImmutable::parse('2026-06-11T00:00:00Z'));
    ...
    Billify::changePlan($item, $s, DowngradePolicy::Discard, CarbonImmutable::parse('2026-06-21T00:00:00Z'));
    $item = $item->fresh()->setRelation('subscription', $sub);
    ...
    Billify::renew($sub, CarbonImmutable::parse('2026-07-01T00:00:00Z'));
...
it('simulates a full hourly cycle S → M → S billed on usage', function () {
    ...
    Billify::recordUsage($item, 'hours', 100, CarbonImmutable::parse('2026-06-05T00:00:00Z'));

    Billify::changePlan($item, $m, DowngradePolicy::Discard, CarbonImmutable::parse('2026-06-10T00:00:00Z'));
    $item = $item->fresh()->setRelation('subscription', $sub);
    Billify::recordUsage($item, 'hours', 200, CarbonImmutable::parse('2026-06-15T00:00:00Z'));

    Billify::changePlan($item, $s, DowngradePolicy::Discard, CarbonImmutable::parse('2026-06-20T00:00:00Z'));
    $item = $item->fresh()->setRelation('subscription', $sub);
    Billify::recordUsage($item, 'hours', 100, CarbonImmutable::parse('2026-06-25T00:00:00Z'));

    // Close the month — bill each tier's hours at its own rate.
    $charges = Billify::rollupUsage($item, new Period(CarbonImmutable::parse('2026-06-01Z'), CarbonImmutable::parse('2026-07-01Z')));
