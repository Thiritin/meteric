...
function commitItem(int $minor = 1000): SubscriptionItem
{
    ...
    $sub = Billify::subscribe()->account($acc)->at(CarbonImmutable::parse('2026-06-01T00:00:00Z'))->add($price, 1)->create();
...
it('bills the upfront and records the commitment', function () {
    ...
    $commitment = Billify::commit($item, Interval::Year, 1, Money::of('120.00', 'EUR'), Money::of('8.00', 'EUR'), at: CarbonImmutable::parse('2026-06-01T00:00:00Z'));
...
it('applies the committed rate on renewal', function () {
    ...
    Billify::commit($item, Interval::Year, 1, Money::of('0', 'EUR'), Money::of('8.00', 'EUR'), at: CarbonImmutable::parse('2026-06-01T00:00:00Z'));
...
it('charges a fixed early termination fee', function () {
    $item = commitItem();
    $commitment = Billify::commit($item, Interval::Year, 1, Money::of('0', 'EUR'), Money::of('8.00', 'EUR'),
        earlyTerm: ['fee_minor' => 5000], at: CarbonImmutable::parse('2026-06-01T00:00:00Z'));

    $fee = Billify::terminateCommitment($commitment, CarbonImmutable::parse('2026-08-01T00:00:00Z'));
